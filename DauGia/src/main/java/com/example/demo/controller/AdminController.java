...
//    public MyUserDetails myUserDetails = new  TaiKhoanQuyenService();
    ...
    public String AdminView(@RequestParam("id") int id, Model model, Principal principal) {
        ...
        model.addAttribute("cd", "chưa duyệt!");
...
//        model.addAttribute("appUser", userRoleService.fi )
    ...
    public String delete(@PathVariable int maSanPham, Model model,SanPham sanPham, Principal principal,RedirectAttributes redirectAttributes) {
        NguoiDung nguoiDung = nguoiDungRepo.findByTaiKhoan_TaiKhoan(principal.getName());
        model.addAttribute("nguoiDung", nguoiDung);
        redirectAttributes.addFlashAttribute("mgsxoa1", "Xóa sản phẩm" +sanPham.getTenSanPham() +" thành công!");
    ...
    public String AdminEdit(SanPham sanPham, Model model,RedirectAttributes redirectAttributes, Principal principal) {
        NguoiDung nguoiDung = nguoiDungRepo.findByTaiKhoan_TaiKhoan(principal.getName());
        model.addAttribute("nguoiDung", nguoiDung);
        this.sanPhamService.create(sanPham);
